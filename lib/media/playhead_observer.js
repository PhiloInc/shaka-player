...
shaka.media.PlayheadObserver.prototype.addTimelineRegion = function(
  ...
  var cloneRegionInfo = shaka.media.PlayheadObserver.cloneTimelineRegionInfo_;
  var event = new shaka.util.FakeEvent(
      'timelineregionadded', {detail: cloneRegionInfo(regionInfo)});
  this.onEvent_(event);
    ...
    if (!(isSeek && passedRegion)) {
      if (!wasInside) {
        this.onEvent_(new shaka.util.FakeEvent(
            'timelineregionenter', {'detail': cloneRegionInfo(region.info)}));
      }
      if (!isInside) {
        this.onEvent_(new shaka.util.FakeEvent(
            'timelineregionexit', {'detail': cloneRegionInfo(region.info)}));
      }
    }
...
/**
 * Create a deep clone of a TimelineRegionInfo object.
 *
 * @param {shakaExtern.TimelineRegionInfo} regionInfo
 * @return {shakaExtern.TimelineRegionInfo}
 * @private
 */
shaka.media.PlayheadObserver.cloneTimelineRegionInfo_ = function(regionInfo) {
  var cloneObject = shaka.util.ConfigUtils.cloneObject;
  var regionInfoCopy = cloneObject(regionInfo);
  // cloneObject will not copy <Element>
  if (regionInfo.eventElement &&
      typeof regionInfo.eventElement.cloneNode != 'undefined') {
    regionInfoCopy.eventElement = regionInfo.eventElement.cloneNode(true);
  }
  return regionInfoCopy;
};
