 ...
 */
shaka.media.MediaSourceEngine = function(video, bFrameAdjustment) {
...
shaka.media.MediaSourceEngine.prototype.setAppendWindow_ =
  ...
  const ContentType = shaka.util.ManifestParserUtils.ContentType;
  let fudge = 1 / 25; // one frame, assuming a low framerate
  if (contentType == ContentType.VIDEO &&
    ...
    // we add an extra frame because the bframe count can be variable
    // If bframeAdjustment is 2, and the count at the end is 1, we want to
    // get the extra frame that we would otherwise miss from the PTO adjustment
    fudge = (this.bFrameAdjustment_.bFrameCount + 2) / this.videoFrameRate_;
  }
